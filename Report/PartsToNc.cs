...
namespace NestixReport
{
    partial class MainWindow
    {
        ...
        private void PartsToNcRelationClick(object sender, RoutedEventArgs e)
        {
            ...
            s.Range["A1"].Value2 = "Заказ";
            s.Range["B1"].Value2 = "Секция";
            s.Range["C1"].Value2 = "№ поз.";
            s.Range["D1"].Value2 = "Толщина, мм";
            s.Range["E1"].Value2 = "Марка материала";
            s.Range["F1"].Value2 = "Кол-во";
            s.Range["G1"].Value2 = "Масса 1 шт., кг";
            s.Range["H1"].Value2 = "Общ. масса, кг";
            s.Range["I1"].Value2 = "Карта раскроя";

            s.Range["A1", "I1"].Font.Bold = true;
            s.Range["A1", "I1"].EntireColumn.VerticalAlignment = Microsoft.Office.Interop.Excel.XlVAlign.xlVAlignCenter;
            s.Range["A1", "I1"].EntireColumn.HorizontalAlignment = Microsoft.Office.Interop.Excel.XlHAlign.xlHAlignCenter;

            s.Range["A1", "C1"].EntireColumn.NumberFormat = "@";
            s.Range["G1", "H1"].EntireColumn.NumberFormat = "0.00";
            ...
            {
                s.Range["A" + row].Value2 = p[1];
                s.Range["B" + row].Value2 = p[2];
                s.Range["C" + row].Value2 = p[3];
                s.Range["D" + row].Value2 = p[7];
                s.Range["E" + row].Value2 = p[8];
                s.Range["F" + row].Value2 = p[4];
                s.Range["G" + row].Value2 = p[5];
                s.Range["H" + row].Value2 = p[6];

                var nclist = parts.FindAll(x => x[3] == p[3] && x[2] == p[2] && x[1] == p[1]);
                var nx = nclist.Select(nc => nc[0]).ToArray();
